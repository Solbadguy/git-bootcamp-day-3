...
_одним абзацем — что положили в Initial commit_ - добавил tasks.py и README.md
_почему `Initial commit` оставлен без префикса Conventional Commits_ - префикса нет, тк это первый коммит и создание репозитория.

## Шаг 2. chore: .gitignore
_одним абзацем — какой стек, откуда взяли `.gitignore`_ - использовал toptal-генератор, включил туда пайтон, тк иногда пишу на нем скрипты.

## Шаг 3. feat(tasks): complete_task
_одним абзацем — что добавили в `tasks.py`, почему тип `feat` и почему scope `tasks`_ - тип feat выбран тк в тот момент была добавлена Новая функциональность, это соотв общепринятой таблице типов.
...
Оставшийся hunk из предыдующщего шага, закоммитили обычным git add. Перед коммитом проверили git diff --staged.
...
_Одним абзацем — что добавили в `README.md` (оглавление, секция TODO, code block, `<details>`_ - Добавлены оглавление, секции TODO/Установка/Лицензия и чек-лист в README.md. Создан LAB.md с описанием всех шагов.
...
Выбрал v0.1.0, а не v1.0.0, т.к. это первый рабочий релиз taskmanager, по SemVer минорная версия, которая позволяет добавлять функционал и исправление багов, без изменения API. Чтобы перейти по SemVer на мажорную версию v1.0.0 (первый стабильный публичный релиз), необходимо, протестировать весь функционал заложенный в коде и стабильность API.
Был выбран tag annotated, т.к. он используется для релизных версий и содержит информацию о том, кто и когда выпустил релиз. И GitHub создаёт страницу релиза и архивы для скачивания только из annotated тегов.
